...
const { getChatHistory, storeChatMessages } = require("./history"); // This now includes automatic cleanup!
...
async function geminiAIReply(req, res) {
  try {
    ...
    // This will automatically clear messages older than 24 hours before returning history
      ...
      config: {
        ...
        tools:[{
          functionDeclarations:[
            tools.changeToneFunction,
            tools.saveStatusFunction,
            tools.downloadTikTokVideoFunction,
            tools.downloadInstagramVideoFunction,
            tools.tagAllFunction,
            tools.unlockViewOnceFunction,
            tools.unlockViewOnceToDMFunction,
            tools.turnOnAntilinkFunction,
            tools.turnOffAntilinkFunction,
            tools.startWCGFunction,
            tools.endWCGFunction,
            tools.wcgStatusFunction,
            tools.createStickerFunction,
            tools.searchAnimeFunction,
            tools.tagMessageFunction
          ]
        }]
      },
      history
    ...
    if (response.functionCalls && response.functionCalls.length > 0) {
      ...
      // Store the user message and function call response
      await storeChatMessages(apikey, chatID, "user", message);
      await storeChatMessages(apikey, chatID, "model", `[Function Call: ${functionCall.name}]`);
